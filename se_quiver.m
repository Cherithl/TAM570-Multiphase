function se_quiver(X,Y,U,V,name, T);
hdr
...
for e=1:E
    ...
    contourf(x, y, reshape(T(:, e, :), N1, N1), 80, Linecolor='None'); axis equal; colorbar; colormap('hot'); hold on;
    %mesh(x, y, reshape(T(:, e, :), N1, N1)); hold on;
    %quiver(x,y,u,v,'off','Color', 'k'); hold on;
end
title(name,fs,16);
xlabel('X',fs,16);
ylabel('Y',fs,16);
