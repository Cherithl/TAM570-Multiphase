...
Ax = Lxi*Rx*Ah*Rx'; Bx = Lx2*Rx*Bh*Rx';
...
% LHS = [Bx, -Cn^2*Ax; dt*Ax, 11/6*Bx];
LHS = [11/6*Bx, dt*Ax; zeros(size(Ax)), (6*dt*(Cn^2)/11)*Ax*inv(Bx)*Ax + Bx ];
...
for istep = 1:nsteps; time = istep*dt;
    ...
    rhs2 = Bx * (a1*psi1 + a2*psi2 + a3*psi3) + (6*(Cn^2)/11)*Ax*(b1*phi1 + b2*phi2 + b3*phi3);
    ...
    % Amat = 1/dt * Bx + Cn^2/Pe * Ax * Bxi * Ax;
    % bvec = 1/dt * Bx * phi - 1/Pe * Ax * (phi.^3 - phi);
    % 
    % phi = Amat \ bvec;
